...
int readGADGETBinaryFile(){
  ...
  //--- Random velocities between -1.0 and 1.0 ---
  /*
  for(i=0; i<N_tot; i++){
    part[i].velx = randr(1.0);
    part[i].vely = randr(1.0);
    part[i].velz = randr(1.0);
  }//for i
  */
 ...
 * Function:  velsHist
 * --------------------
 * Computes the histogram for the distribution of velocities for each 
 * particle in each axis. Also, computes the mean and the mode of the 
 * distribution of velocities in each axis. 
 * In order to compute the bins, it also search for the maximum and 
 * minimum values of the velocities in each axis.
 *
 * 
 *
 * 
 *
 *  returns: The histogram data: bins and the stored values in each bin.
 */
/*
double vels_Hist(){
  int i;
  ...
  size_t histBins;
  double *binsRange, *vxBins;
  double deltaBins;
  
  //--- Looking for the maximum and minimum values ---
  //--- of the velocites in each axis ---
  minVel = 1.0e20;
  maxVel = GV.ZERO;

  for(i=0; i< GV.NpTot; i++){
    if (part[i].velx < minVel)
      minVel = part[i].velx;
    
    if (part[i].velx > maxVel)
      maxVel = part[i].velx;
  }//for i

  histBins = 11585; //Square root of the number of particles
  deltaBins = (maxVel - minVel) / ((double) histBins);

  //--- Initializing range and bins ---
  vxBins = (double *)calloc(histBins, sizeof(double));
  binsRange = (double *)calloc(histBins+1, sizeof(double));
  
  for(i=0; i<(histBins+1); i++){
    binsRange[i] = i*deltaBins;
  }//for i

  //--- Computing the histogram ---
  gsl_histogram * vxHist = gsl_histogram_alloc(histBins);
  gsl_histogram_set_ranges(vxHist, binsRange, histBins+1);
  
  //gsl_histogram_set_ranges_uniform(vxHist, minVel, maxVel);
  

  free(vxBins);
  free(binsRange);
  gsl_histogram_free(vxHist);
