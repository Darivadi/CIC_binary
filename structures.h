...
struct Cell{
  ...
  double  momentum_p[3];  // Momentum in the cells
  /* The W_count is a counter of the weight function for 
     every cell. This is necessary in order to compute 
     the velocities assigment in each cell. Once the 
     velocities in the cells have been assigned, one must 
     divide this velocity cell value by its respective 
     W_count */
  double    W_count; 
  double pos[3]; // Cell's position
}*cells;
