...
int write_binary_parts(void)
{
      ...
      if(part[i].posz <= 40.0)
 {
   ...
   pos_aux[1] = part[i].posy;
   pos_aux[2] = part[i].posz;
...
# 93 "CIC_noperiodic.c"
  ...
  sumaVelModule = sqrt(sumaVelPart[0]*sumaVelPart[0] + sumaVelPart[1]*sumaVelPart[1] + sumaVelPart[2]*sumaVelPart[2]);
...
# 260 "CIC_noperiodic.c"
       }
     }
   }

 }
# 276 "CIC_noperiodic.c"
      }
    }
  }
  ...
  norm_factor = GV.aSF * (GV.NGRID3 / GV.NpTot);
  for(i=0; i<GV.NGRID; i++){
    for(j=0; j<GV.NGRID; j++){
      for(k=0; k<GV.NGRID; k++){

 index = (k)+GV.NGRID*((j)+GV.NGRID*(i));
# 300 "CIC_noperiodic.c"
